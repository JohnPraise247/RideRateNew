...
const NavBar = {
    view: ({ state: { hash = m.route.param("urlA") || "dashboard", title = hash.charAt(0).toUpperCase() + hash.slice(1), usertype = m.route.get().slice(1, 6) }})=>{
        return m(".navbar.flex.justify-between.bg-base-100.z-10.shadow-md", [
            m("div",
                [
                    ...
                    m("h1.text-xl.font-semibold.ml-2.md:text-2xl", 
                        hash == "dashboard" || hash == "locations" || hash == "rates" || hash == "settings" || (hash == "users" && usertype == "admin") || (hash == "analytics" && usertype == "admin") ? title
                        : "Page not found"
                    )
                ]
            ),
